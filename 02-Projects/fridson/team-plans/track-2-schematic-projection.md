...
> ✅ **SHIPPED + PUSHED 2026-06-28** (`6b50e0c` on `origin/main`). `/projection` (stage view: phase rail + schematic + step-card feed) and `/schematic` (per-floor selector + open-report markers + pulsing marker). Scripted mock emitter plays the full 8-step sequence (~54s, pinpoint at 1.5s); real Supabase `events` channel is flag/URL gated. tsc + build pass. **Remaining is manual:** apply migrations + deploy edge functions, then on the stage laptop open `/projection?feed=real` and verify Realtime on `events`.
...
- [x] Render the floorplan; place a **marker** using `location.x/y` × rendered size (per [[INTERFACES]] §2) ✅ — per-floor selector + open-report markers on `/schematic` (pushed `6b50e0c`)
...
- [ ] *(stretch, only if time)* light per-ticket "context record" view for Minute 2 — ✅ handled by Phase 5 (`WorkRecordPane` stakeholder lenses); script from Track 4
